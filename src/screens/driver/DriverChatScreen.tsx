...
const quickReplies = ['On my way', 'Arrived at pickup', 'Please confirm address'];

type DriverChatRoute = RouteProp<DriverAccountStackParamList, 'DriverChat'>;

const DriverChatScreen = () => {
  useHideTabs('DriverTabs');
  const navigation = useNavigation<NativeStackNavigationProp<DriverAccountStackParamList>>();
  const route = useRoute<DriverChatRoute>();
  ...
  const handleSend = (rawText?: string) => {
    ...
    if (!payload) return;
    sendMessage(orderId, payload, 'driver');
    setText('');
        ...
        <View style={[layout.container, { flex: 1, paddingTop: 12 }]}> 
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <View style={{ alignItems: isDriver ? 'flex-end' : 'flex-start' }}>
                  <View
                    style={{
                      maxWidth: '85%',
                      borderRadius: 12,
                      ...
                      paddingVertical: 9,
                      backgroundColor: isDriver ? colors.dark : colors.gray100,
                    }}
                  >
                    <Text style={{ fontFamily: typography.fonts.regular, color: isDriver ? colors.white : colors.dark }}>
                      {item.text}
                    </Text>
                  </View>
                </View>
